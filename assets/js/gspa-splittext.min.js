...
!(function (D, u) {
	...
	function z(D, u, e) {
		var t = D.nodeType;
		if (1 === t || 9 === t || 11 === t)
			for (D = D.firstChild; D; D = D.nextSibling) z(D, u, e);
	...
	function E(e, t, F, i, n, s, E) {
		var r,
			...
			w = D("paddingLeft", b),
			S = -999,
			_ = D("borderBottomWidth", b) + D("borderTopWidth", b),
			m = D("borderLeftWidth", b) + D("borderRightWidth", b),
			T = D("paddingTop", b) + D("paddingBottom", b),
			N = D("paddingLeft", b) + D("paddingRight", b),
			L = D("fontSize", b) * (t.lineThreshold || 0.2),
			W = b.textAlign,
			O = [],
			H = [],
			j = [],
		...
		for (
			I && (q = q.split("++").join("")),
				...
				l = (o = e.getElementsByTagName("*")).length,
				d = [],
				r = 0;
			r < l;
			r++
				...
				((a = (p = d[r]).parentNode === e) || $ || (G && !P)) &&
					...
					R &&
						(((p._isSplit && a) ||
							...
							(h.push(p), (p._x -= w), B(p, e, k) && (p._wordEnd = !0)),
						"BR" === p.nodeName &&
							((p.nextSibling && "BR" === p.nextSibling.nodeName) || 0 === r) &&
							R.push([])));
		for (r = 0; r < l; r++)
			if (((a = (p = d[r]).parentNode === e), "BR" !== p.nodeName))
				if (
					($ &&
						((c = p.style),
					...
					if (p._isSplit)
						...
						p.parentNode._isSplit
							? ((p._parent = p.parentNode),
							  ...
							  l--)
							: a ||
							  ((y = !p.nextSibling && B(p.parentNode, e, k)),
							  p.parentNode._parent && p.parentNode._parent.appendChild(p),
							  y && p.parentNode.appendChild(U.createTextNode(" ")),
			...
			for (f = " " === k && (!$ || (!P && !G)), r = 0; r < R.length; r++) {
				for (
					...
					"BR" !== h[o].nodeName &&
						...
						$ &&
							(0 === o &&
								((g.style.top = p._y + "px"), (g.style.left = w + y + "px")),
							(p.style.top = "0px"),
							y && (p.style.left = p._x - y + "px")));
				0 === l
					? (g.innerHTML = "&nbsp;")
					: P || G || (C(g), z(g, String.fromCharCode(160), " ")),
					$ && ((g.style.width = x), (g.style.height = p._h + "px")),
					e.appendChild(g);
			}
			e.style.cssText = c;
		}
		$ &&
			(E > e.clientHeight &&
				((e.style.height = E - T + "px"),
				e.clientHeight < E && (e.style.height = E + _ + "px")),
			s > e.clientWidth &&
				((e.style.width = s - N + "px"),
				e.clientWidth < s && (e.style.width = s + m + "px"))),
			K && (Q ? (e.style.display = Q) : e.style.removeProperty("display")),
			A(F, O),
			P && A(i, H),
			A(n, j);
	}
	function F(D, u, e, t) {
		var F,
			...
			p = ~(u.type || u.split || "chars,words,lines").indexOf("chars"),
			d = v(u),
			a = u.wordDelimiter || " ",
			h = " " !== a ? "" : d ? "&#173; " : " ",
			f = "</" + l + ">",
		...
		for (
			...
			if (((E = F.charAt(C)), c && (o = c(F.substr(C), u.specialChars))))
				(E = F.substr(C, o || 1)),
					(i += p && " " !== E ? t() + E + "</" + l + ">" : E),
					(C += o - 1);
			else if (E === a && F.charAt(C - 1) !== a && C) {
				for (i += B ? f : "", B = 0; F.charAt(C + 1) === a; ) (i += h), C++;
				C === s - 1
					? (i += h)
					: ")" !== F.charAt(C + 1) && ((i += h + e()), (B = 1));
			} else
				"{" === E && "{{LT}}" === F.substr(C, 6)
					? ((i += p ? t() + "{{LT}}</" + l + ">" : "{{LT}}"), (C += 5))
					: (55296 <= E.charCodeAt(0) && E.charCodeAt(0) <= 56319) ||
					  (65024 <= F.charCodeAt(C + 1) && F.charCodeAt(C + 1) <= 65039)
					? ((n = ((F.substr(C, 12).split(x) || [])[1] || "").length || 2),
					  (i +=
							p && " " !== E
								? t() + F.substr(C, n) + "</" + l + ">"
								: F.substr(C, n)),
	...
	function G(D, e, t, i) {
		var C,
			...
			r = v(e);
		if (3 !== D.nodeType || 1 < E) {
			for (e.absolute = !1, C = 0; C < E; C++)
				((n = s[C])._next = n._isFirst = n._parent = n._wordEnd = null),
					(3 === n.nodeType && !/\S+/.test(n.nodeValue)) ||
						...
						(n._isSplit = !0),
						G(n, e, t, i));
			return (e.absolute = r), void (D._isSplit = !0);
		}
		F(D, e, t, i);
	}
	var U,
		...
		S = /(?:\s\s+)/g,
		l = "SplitText",
		d = p(103, 114, 101, 101, 110, 115, 111, 99, 107, 46, 99, 111, 109),
		a = p(103, 115, 97, 112, 46, 99, 111, 109),
				...
				setTimeout(function checkWarn() {
					if (u)
						if (
							...
							var D = "object" == typeof n ? n : u && window.gsap;
							u &&
								window.console &&
								!window._gsapWarned &&
								"object" == typeof D &&
			...
			(((r = SplitText.prototype).split = function split(D) {
				...
				for (
					...
					(t = this.elements[F]),
						...
						(e = t.clientWidth),
						G(t, D, C, n),
						E(t, D, this.chars, this.words, this.lines, e, u);
				return (
					this.chars.reverse(),
					this.words.reverse(),
					this.lines.reverse(),
	...
	(c.version = "3.12.3"), (c.register = m), (e.SplitText = c), (e.default = c);
